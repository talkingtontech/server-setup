#!/bin/bash
# IPTables Country Block Script v1.4.7
...
function UPDATEZONECACHE() {
  for c in $ISO; do
    CACHEFILENAME="$DLPROVIDER-$c.zone"
    CACHEFILE="$ZONEROOTDIR/$CACHEFILENAME"

    if [ $DLPROVIDER == "ipdeny" ]; then
      DLZONECMD="$WGETBIN -O $CACHEFILE $DLROOTIPDENY/$c.zone"
    elif [ $DLPROVIDER == "ipinfodb" ]; then
      DLZONECMD="$WGETBIN -O $CACHEFILE $DLROOTIPINFODB?country=$c"
    fi

    if [ $ZONECACHEDAY = 0 ]; then
      CACHECLEANCMD="$FINDBIN $ZONEROOTDIR -name \"$CACHEFILENAME\" -type f -print0"
    else
      CACHECLEANCMD="$FINDBIN $ZONEROOTDIR -name \"$CACHEFILENAME\" -type f -ctime +$ZONECACHEDAY -print0"
    fi
...
function CREATEIPTCONFIG() {
  ...
  for c in $ISO; do
    CACHEFILE="$ZONEROOTDIR/$DLPROVIDER-$c.zone"

    LOGDROPMSG="iptables: countryblock-$c: "
    CHAINDROP="$CBCHAIN-$c"

    printf ":$CHAINDROP - [0:0]\n" >> $CBRESTORE

    if [ $LOGDROP = true ]; then
      printf "%s $CHAINDROP -j LOG --log-prefix \"$LOGDROPMSG\"\n" "-A" >> $RESTORETMP
    fi

    printf "%s $CHAINDROP -j DROP\n" "-A" >> $RESTORETMP

    if [ -f $CACHEFILE ]; then
      BADIPS=$($EGREPBIN -v "^#|^$" $CACHEFILE)

      for ipblock in $BADIPS; do
        OCTET=`echo $ipblock | cut -f 1 -d '.'`
        OCTETCHAIN="$CBCHAIN-$OCTET"
        CHAINTEST=`grep -c :$OCTETCHAIN $CBRESTORE`

        if [ $CHAINTEST = 0 ] ; then
          NOTICE "Creating chain for octet $OCTET"
          printf ":$OCTETCHAIN - [0:0]\n" >> $CBRESTORE

          SRCIP="$OCTET.0.0.0/8"

          printf "%s $CBCHAIN -s $SRCIP -j $OCTETCHAIN\n" "-A" >> $RESTORETMP
        fi

        printf "  Adding rule for %s to chain for octet %s\n" $ipblock $OCTET
        printf "%s $OCTETCHAIN -s $ipblock -j $CHAINDROP\n" "-A" >> $RESTORETMP
      done
    else
      NOTICE "Missing zone cache file: $CACHEFILE"
    fi
  done

  cat $RESTORETMP >> $CBRESTORE && rm -f $RESTORETMP
...
FORCERUNCMD mkdir -p -v $ZONEROOTDIR
